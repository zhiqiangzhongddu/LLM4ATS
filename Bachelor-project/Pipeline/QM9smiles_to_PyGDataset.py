...
ds = PygGraphPropPredDataset(name = "QM9", root = "data/QM9")
print(ds.y[0])
exit()
df = pd.read_csv("/home/daniel.thoby/LLMaPretrain/Bachelor-project/Pipeline/data/raw/gdb9.sdf.csv")
PEMP_props_idx = [3, 4, 7, 10, 14]
df = df.drop(["mol_id"], axis=1)
res = []
for i in range(19):
    if i not in PEMP_props_idx:
        res.append(i)
df = df.drop(df.columns[res], axis=1)
print(df.head())
lst = df.values.tolist()
print(lst[0])
f = open("/home/daniel.thoby/LLMaPretrain/Bachelor-project/Pipeline/data/QM9/QM9_pyg/raw/graph-label.csv", "w")
