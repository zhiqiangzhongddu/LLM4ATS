...
def calc_RDKit_desc(mol, desc_name_list): 
    ...
    Descriptors_lst = ["HeavyAtomMolWt", "NOCount", "NumValenceElectrons"]
    ...
    list_of_uncalc_desc = ["Topliss fragments", "FractionCSP3", "NumAmideBonds", "NumBridgeheadAtoms", "BCUT2D", "NumAromaticRings", "NumAliphaticRings", "NumSaturatedRings", "MolWT", "MQNs", "ExactMolWT", "Autocorr2D", "NumSpiroAtoms", "Phi"]
...
def get_RDKit_values(mol_list_in, desc_list_in):
    def helper(mol_list, desc_list, i):
        if i >= len(mol_list_in):
            return []
        return [calc_RDKit_desc(mol_list[i], desc_list)] + helper(mol_list, desc_list, i+1)
    return helper(mol_list_in, desc_list_in, 0)
