    ...
    def __init__(self, num_layer, emb_dim, drop_ratio = 0.5, JK = "last", residual = False, gnn_type = 'gin'):
        ...
        for layer in range(num_layer):
            if gnn_type == 'gin':
                ...
                self.convs.append(GCNConv(in_channels=emb_dim, out_channels=emb_dim, cached=True))
            else:
...
class GNN_node_Virtualnode(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, drop_ratio = 0.5, JK = "last", residual = False, gnn_type = 'gin'):
        ...
        for layer in range(num_layer - 1):
            self.mlp_virtualnode_list.append(
                torch.nn.Sequential(
                    torch.nn.Linear(emb_dim, 2 * emb_dim), torch.nn.BatchNorm1d(2 * emb_dim),
                    torch.nn.ReLU(),
    ...
    def forward(self, batched_data):
        ...
        if str(edge_index.device) == "cpu": x = x.type(torch.LongTensor)
        else: x = x.type(torch.cuda.LongTensor)
        h_list = [self.atom_encoder(x)]
        for layer in range(self.num_layer):
